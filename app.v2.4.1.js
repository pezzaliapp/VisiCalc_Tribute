...
formulaEl.addEventListener('keydown', function(e){
  ...
  document.addEventListener('keydown', function(e){
    // If we're typing inside the formula input, don't hijack keys
    ...
    if(e.key === 'ArrowRight'){ c=Math.min(COLS-1,c+1); }
      ...
      // Start typing in formula bar without duplicating the first char
      e.preventDefault();
      formulaEl.value = e.key;
      formulaEl.focus();
      try { var L = formulaEl.value.length; formulaEl.setSelectionRange(L, L); } catch(_){}
      return;
    }
    else return;
  ...
  btnHelp.addEventListener('click', function(){
    try{ window.open('./formulas_guide.html', '_blank'); return; }catch(_){ }
